...
try:
	...
	stdin, stdout, stderr = ssh.exec_command("sudo python MaggotBot/threading_phototaxis.py")
	while True:
		...
		while True:
			...
			if len(bndcnts) > 0 and len(objcnts) > 0:
				...
				# move 1 initiates phototaxis on Pi
				if move == 1:
				...
				# move 2 does nothing during phototaxis
				# stops phototaxis once robot leaves boundary
				if move == 2:
				...
				# move 3 tells robot how to get back to center
				if move == 3:
					r = get_dist(objcenter, bndcenter)
			...
			# shutdown if robot is outside view of camera		
			#else:
